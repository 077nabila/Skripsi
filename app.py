...
# =========================
    ...
    ax.plot(df["Tanggal"], df["RR"], label="Sebelum")
    ax.plot(df_interp["Tanggal"], df_interp["RR"], label="Sesudah")

    ax.set_title("Perbandingan Sebelum vs Sesudah Interpolasi")
...
# =========================
    ...
    fig, ax = plt.subplots(figsize=(14, 5))
    ax.plot(df_scaled["Tanggal"], df_scaled["RR"])
    ax.set_title("RR Setelah Normalisasi")
    ax.set_xlabel("Tanggal")
...
# =========================
    ...
    if model is None or scaler is None or x_test is None or y_test is None or df is None:
        st.error("Pastikan semua proses sudah dilakukan.")
        st.stop()
